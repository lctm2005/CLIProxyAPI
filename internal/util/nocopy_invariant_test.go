...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	...
	"internal/runtime/executor/traecli_executor.go":         {1, "copies random UUID bytes into a freshly allocated local buffer before formatting; no payload reader can observe it"},
